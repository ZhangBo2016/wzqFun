...
function drawRect() {
    ...
    context.strokeText('游戏规则：玩家执黑色', 780, 400);
    context.strokeText('棋子先手，电脑执白色棋子', 750, 420);
    context.strokeText('后手，任何一方形成五子连', 750, 440);
    context.strokeText('珠，游戏终止。', 750, 460);
    //棋盘纵横线
    for (var i = 1; i < 16; i++) {
        context.beginPath();
...
var panelInit = function(){
    ...
    var pre_btn = {
        execute: function(){
            var cookieDatas = getCookie(color).split(";")
            cookieDatas.pop();
            delCookie(color);
            if(cookieDatas.length!=0) {
                cookieDatas = cookieDatas.join(";");
                setCookie(color, cookieDatas, 30);
            };
            location.reload();
        }
    };
